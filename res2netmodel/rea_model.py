...
class Res2Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x_e = [x_layer3, x_layer2, x_layer1, x_layer0]
        return x_e
...
class SE(nn.Module):
    ...
    def forward(self, x):
        x = self.conv(x)  # 通道缩减
        b, c, _, _ = x.size()
        scale = self.global_pool(x).view(b, c)
        scale = self.fc2(F.relu(self.fc1(scale))).view(b, c, 1, 1)
        return x * torch.sigmoid(scale)  # 通道加权
...
class decoder(nn.Module):
    ...
    def forward(self, x, x_e):
        ...
        print(f"x_e[0] shape: {x_e[0].shape}")
        print(f"res16x shape: {res16x.shape}")
        res8x = self.C1(x_e[1])
        print(f"x_e[1] shape: {x_e[1].shape}")
        print(f"res8x shape: {res8x.shape}")
        res4x = self.C2(x_e[2])
        print(f"x_e[2] shape: {x_e[2].shape}")
        print(f"res4x shape: {res4x.shape}")
        res2x = self.C3(x_e[3])
        print(f"x_e[3] shape: {x_e[3].shape}")
        print(f"res2x shape: {res2x.shape}")
...
class MX(nn.Module):
    def __init__(self):
        ...
        self.encoder = Res2Net(Bottle2neck, [3, 4, 23, 3])
